## the grand refactoring of 5/2021:
# split up into mulitple files - api/getting data, calculating, arbitrage, models? or maybe keep it all here
# find out what's actually generally useful and what's only useful for certain markets
# make this all readable, note where code should be archived
...
## 3) get a VM and have it send emails daily
# ---- arbitrage
# ---- 538 approval rating
...
class odds_calc:
    ...
    def real_odds(self):
        ...
        return self.odds / new_total

    def inv_real_odds(self):
        return (self.odds * 0.9) / (1 - self.odds * 0.1)
    ...
    def ror_arbitrage(self):
        ...
        # mean
        profit_tax = [(1 - np.mean(self.odds)) * (len(self.odds) - 1) * 0.1]

        # if lowest probability happens
        profit_tax.append(
            (1 - (np.sum(self.odds) - np.min(self.odds)) / (len(self.odds) - 1))
            * (len(self.odds) - 1)
            * 0.1
        )

        # if highest probability happens
        profit_tax.append(
            (1 - (np.sum(self.odds) - np.max(self.odds)) / (len(self.odds) - 1))
            * (len(self.odds) - 1)
            * 0.1
        )
        ...
        withdrawal_tax = (revenue - profit_tax) * 0.05 if self.withdrawal_tax else 0
        return 100 * (((revenue - (withdrawal_tax + profit_tax)) / cost) - 1)
...
# function to parse a column of a json array into nice rows for a dataframe


def json_normalize_merge(df, rsuffix, normalize_col):
    dfc = df.copy()
    return (
        pd.concat({i: pd.json_normalize(x) for i, x in dfc.pop(normalize_col).items()})
        .reset_index(level=1, drop=True)
        .join(
            dfc, rsuffix=rsuffix
        )  # .drop(columns=['id', 'image', 'name', 'shortName', 'status'])
...
def pull_predictit_new():
    p = requests.get("https://www.predictit.org/api/marketdata/all/")

    df = pd.json_normalize(p.json()["markets"])
    return json_normalize_merge(df, rsuffix="_contract", normalize_col="contracts")
...
class predictIt:
    ...
    def arbitrage_possibles(self, withdrawal_tax=True, all_profitable=False):
        ...
        threshold = 0.95 if withdrawal_tax else 1
        df["inv_buy_no"] = 1 - df["bestBuyNoCost"]

        possibles = (
            df[df["bestBuyNoCost"] < threshold]
            .groupby(["contract_id", "contract_name"])["inv_buy_no"]
            .sum()
            .sort_values(ascending=False)
        )

        profit_threshold = 1 if all_profitable else 1.11
        return possibles[possibles > profit_threshold]

    def arbitrage_roi(self):
        df = self.df
        arb_possibles = predictIt(df).arbitrage_possibles(
            withdrawal_tax=False, all_profitable=True
        )
        ...
        ROIs["contract_name"] = arb_possibles.index.get_level_values(
            "contract_name"
        ).tolist()
        ...
        ROIs = ROIs.merge(
            self.df[["contract_name", "contract_id", "dateEnd"]].drop_duplicates()
        )  # self

        ROIs["time_left"] = (
            pd.to_datetime(ROIs["dateEnd"], errors="coerce") - datetime.datetime.now()
        )

        ROIs["annualized_return"] = ROIs["mean"] / (
            ROIs["time_left"] / datetime.timedelta(365)
        )
    ...
    def paired_outcome_arbitrage(self, paired_outcomes):
        ...
        for market_name, market_id in paired_outcomes.items():
            ...
            paired = df[df["id"].isin(market_id)][
                ["id", "bestBuyYesCost", "bestBuyNoCost"]
            ].reset_index()
            # print(paired)
            ...
            holder["avg_profit_tax"] = 0.1 * (1 - holder["best_price"] / 2)
            holder["raw_profit"] = 1 - holder["best_price"]
            holder["post_tax_profit"] = 1 - (
                holder["best_price"] + holder["avg_profit_tax"]
            )
            paired_df = paired_df.append(holder).sort_values(
                by=["post_tax_profit"], ascending=False
            )
    ...
    def gen_538_cost_df(self):
        ...
        if len(ar["contract_id"].unique()) == 1:
            return self._extracted_from_gen_538_cost_df_6(ar)
        print("multiple possible markets:")
        print(ar["contract_id"].unique())
        print(ar["contract_name"].unique())

    def _extracted_from_gen_538_cost_df_6(self, ar):
        r = (
            ar["name"]
            .str.replace("\%", "")
            .str.replace(" to ", ",")
            .str.replace(" or lower", "")
            .str.replace(" or higher", "")
        )
        r[0] = "00.0," + r[0]
        ...
        cost_df["real_yes_cost"] = cost_df["bestBuyYesCost"].apply(
            lambda x: odds_calc(x).real_odds()
        )

        cost_df["real_no_cost"] = cost_df["bestBuyNoCost"].apply(
            lambda x: odds_calc(x).real_odds()
        )
...
class approval_538:
    def __init__(self, df):
        ...
        ndf = pd.melt(
            df, id_vars=["ds", "y", "timestamp"], var_name="lag", value_name="pred"
        )
    ...
    def cost_probs(self, method="average", buy_margin=0.05):
        ar_data = self.df
        ...
        if datediff_index < 0:
            raise ValueError(
                """It is the day of the market close so don't use the model"""
            )

        if method == "model":
            dist = self._extracted_from_cost_probs_20(datediff_index, most_recent_ar)
        else:
            sd_to_use = st_devs.iloc[datediff_index]
            dist = scipy.stats.norm(most_recent_ar, sd_to_use)

        ## because predictit rounds
        cost_df["actual_lower"] = cost_df["lower"] - 0.05
        cost_df["actual_upper"] = cost_df["upper"] + 0.05

        ## adding the chance each range happens
        cost_df["prob"] = self.cdf_probs(
            dist, lower=cost_df["actual_lower"], upper=cost_df["actual_upper"]
        )
        ...
        cd["buy_yes_to"] = (
            np.floor(100 * odds_calc(cd["prob"] - buy_margin).inv_real_odds()) / 100
        )
        cd["buy_no_to"] = (
            np.ceil(100 * odds_calc(1 - cd["prob"] - buy_margin).inv_real_odds()) / 100
        )
        cd["buy_yes_to"] = cd["buy_yes_to"].where(cd["yes_margin"] >= 0.05)
        cd["buy_no_to"] = cd["buy_no_to"].where(cd["no_margin"] >= 0.05)
        ...
        hours_since_update = round(
            (datetime.datetime.now() - pd.to_datetime(ar_data["timestamp"][0])).seconds
            / (60 * 60),
            1,
        )
        ...
        print(
            str(days_off.days)
            + " days out, current rating is "
            + str(round(most_recent_ar, 2))
            + ", last update posted "
            + str(hours_since_update)
            + " hours ago at "
            + ar_data["timestamp"][0]
        )
    ...
    def _extracted_from_cost_probs_20(self, datediff_index, most_recent_ar):
        ...
        test_df = self.prep_model_df(
            lag_order=for_model["lag"].iloc[datediff_index], shift=False
        )
        test_df = test_df.iloc[-1:]
        test_df[f"lag{lag_order}"] = most_recent_ar

        estimate = self.fit_model(
            train_df,
            test_df,
        ...
        return scipy.stats.norm(fordist, sd_to_use)

    @staticmethod
    def gen_shift(daily_std, days_until_end, num_sims):
        return np.random.normal(loc=0, scale=daily_std, size=days_until_end * num_sims)
    ...
    def hacky_sim_probs(sim_max, lower, upper):
        ...
        for row in range(len(lower)):
            toappend = len(
                [x for x in sim_max if x >= lower[row] and x < upper[row]]
            ) / len(sim_max)
            ans.append(toappend)
    ...
    def max_ar_probs(self, method="simulation", num_sims=1000):
        ...
        if method == "simulation":
            ...
            dates = (
                pd.date_range(today, periods=days_until_end).to_pydatetime().tolist()
            )
            ...
            simulated_maxes = (
                z.groupby("sim_number")["simulated_ar"].max().sort_values()
            )

            highest_so_far = ar_538[ar_538["ds"] > datetime.datetime(2020, 3, 31)][
                "y"
            ].max()

            ## finding the maxes
            sim_max = [
                elem if elem > highest_so_far else highest_so_far
                for elem in simulated_maxes
            ]

            ## adding the probabilities to the df
            max_df["prob"] = self.hacky_sim_probs(
                sim_max, lower=max_df["lower"] - 0.05, upper=max_df["upper"] + 0.05
            )
        else:
            ...
            draw_from = [
                elem if elem > best_so_far else best_so_far for elem in draw_from
            ]

            max_df["prob"] = self.hacky_sim_probs(
                draw_from, max_df["lower"], max_df["upper"]
            )
        ...
        ## printing the string
        hours_since_update = round(
            (datetime.datetime.now() - pd.to_datetime(df["timestamp"][0])).seconds
            / (60 * 60),
            1,
        )
...
def merge_preds(format="wide"):
    ...
    if format == "wide":
        both_preds = pd.merge(
            model_preds,
            avg_preds,
            left_on=["lower", "upper", "bestBuyYesCost", "bestBuyNoCost"],
            right_on=["lower", "upper", "bestBuyYesCost", "bestBuyNoCost"],
            suffixes=["_model", "_avg"],
        )
    ...
    if format == "long":
        ...
        both_preds = both_preds.melt(
            id_vars=["lower", "upper", "bestBuyYesCost", "bestBuyNoCost", "method"],
            value_vars=["yes_margin", "no_margin"],
            var_name="value_type",
            value_name="margin",
        )
...
alt.Chart(p[p["margin"] > 0]).mark_bar().encode(
    x="range", y="margin", color="value_type", facet="method"
)
...
def list_contests(df):
    uqs = (
        df.groupby(["contract_name", "contract_id"])
        .size()
        .reset_index()
